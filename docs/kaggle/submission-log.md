...
| SUB-003 | *(pending)* | EXP-002 | `k02_categoricals` @ *(commit)* | v2a: + categorical encodings (label + freq + email split) | *(pending)* | *(pending)* | private ≥ 0.9077 (H2: +0.020 over SUB-002) | *(pending)* | *(pending)* |
